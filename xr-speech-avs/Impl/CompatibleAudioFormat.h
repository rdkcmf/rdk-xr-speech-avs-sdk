    ...
    namespace AudioFormatCompatibility {
        ...
        static bool IsCompatible(alexaClientSDK::avsCommon::utils::AudioFormat other)
        {
            if (ENCODING != other.encoding) {
                XLOGD_ERROR("Incompatible audio format encoding");
                return false;
            }
            if (ENDIANESS != other.endianness) {
                XLOGD_ERROR("Incompatible audio format endianess");
                return false;
            }
            if (SAMPLE_RATE_HZ != other.sampleRateHz) {
                XLOGD_ERROR("Incompatible audio format sample rate");
                return false;
            }
            if (SAMPLE_SIZE_IN_BITS != other.sampleSizeInBits) {
                XLOGD_ERROR("Incompatible audio format sample size in bits");
                return false;
            }
            if (NUM_CHANNELS != other.numChannels) {
                XLOGD_ERROR("Incompatible audio format number of channels");
                return false;
            }
